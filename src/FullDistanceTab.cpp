...
static QString readSavedDeviceModeFull(const AppConfig& cfg)
{
    QSettings s(uiIniPathFull(), QSettings::IniFormat);
    return cfg.normalizeDeviceMode(s.value("ui/current_device_mode", "auto").toString());
}

static void saveDeviceModeFull(const AppConfig& cfg, const QString& mode)
{
    QSettings s(uiIniPathFull(), QSettings::IniFormat);
    ...
    : QWidget(parent), m_cfg(cfg), m_appDir(appDir)
{
    ...
    root->addWidget(scroll, 1);
    {
        ...
        QString deviceNote;
        const QString lastDevice = m_cfg.effectiveDeviceMode(m_appDir, readSavedDeviceModeFull(m_cfg), &deviceNote);
...
void FullDistanceTab::setDeviceMode(const QString& mode) {
    if (!m_device) return;
    ...
    saveDeviceModeFull(m_cfg, m_device->currentText());
}

QString FullDistanceTab::currentDeviceMode() const {
...
bool FullDistanceTab::runPreviewTaskRaw(const QString& inputPath, QImage& outImage, QJsonObject& outExifRoot,
                                        QString& err)
{
    outImage = QImage();
    ...
    if (!QFileInfo(runner).exists())
    {
        err = "runner.py не найден: " + runner;
        return false;
    }
    if (!QFileInfo(basePyCfg).exists())
    {
        err = "python config не найден: " + basePyCfg;
        return false;
    }
    if (!QFileInfo(inputPath).exists())
    {
        err = "input не найден: " + inputPath;
        return false;
    }
    ...
    {
        err = "Не удалось очистить temp preview dir: " + workDir;
        return false;
    }
    if (!QDir().mkpath(workDir))
    {
        err = "Не удалось создать temp preview dir: " + workDir;
        return false;
    }
    ...
    if (w <= 0 || h <= 0 || stride <= 0)
    {
        err = "preview: неверные размеры";
        wd.removeRecursively();
        return false;
    }
    if (fmt != "RGBA8888")
    {
        err = "preview: неподдерживаемый формат: " + fmt;
        wd.removeRecursively();
        return false;
    }
    if (rawPath.isEmpty() || !QFileInfo(rawPath).exists())
    {
        err = "preview: raw файл не найден: " + rawPath;
        wd.removeRecursively();
        return false;
    }
    ...
    {
        err = "preview: не открыть raw: " + rawPath;
        wd.removeRecursively();
        return false;
    }
    ...
    if (bytes.size() < need)
    {
        err = "preview: raw меньше ожидаемого";
        wd.removeRecursively();
        return false;
    }

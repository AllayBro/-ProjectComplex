...
ClustersTab::ClustersTab(const AppConfig& cfg, const QString& appDir, QWidget* parent)
    ...
    connect(m_browseYolo, &QPushButton::clicked, this, [this] {
        ...
        s.setValue("ui/last_yolo_import_dir", QFileInfo(selected).absolutePath());

        QString importErr;
        const QString imported = importYoloModelToDir(selected, ydir, &importErr);
        if (imported.isEmpty()) {
            QMessageBox::warning(this, "YOLO", importErr.isEmpty() ? "Не удалось подключить модель YOLO." : importErr);
            return;
        }
